...
st.set_page_config(page_title="Macro Snapshot v28.0", layout="wide")

# --- TABS SETUP ---
tab_main, tab_news = st.tabs(["📊 Market Analysis", "📰 Live Economics & Wealth"])
...
def load_market_data(ticker_list):
    df = yf.download(ticker_list + ['GLD'], period="3y")['Close']
...
with tab_main:
    if not raw_data.empty:
        ...
        st.divider()
        st.subheader("3. Macro Command Center: Distilled Dials & Specific Audits")
        ...
        anchors = [('TLT', "📉 TLT (Bonds)"), ('^TNX', "🏛️ 10Y Yield"), ('DX-Y.NYB', "💵 DXY (Dollar)"), ('MSTR', "₿ Crypto (MSTR)"), ('MSFT', "💻 Tech (MSFT)"), ('NVDA', "📟 Semis (NVDA)"), ('XLE', "🔥 Energy (XLE)"), ('JPM', "🏦 Banks (JPM)"), ('WMT', "🛡️ Defensive (WMT)"), ('^VIX', "💀 VIX (Fear)")]
        ...
        for ticker, row_title in anchors:
            ...
            p_df = pd.DataFrame({
                "Period": ["1D", "1M", "3M", "6M"],
                "Perf %": [get_perf_and_trend(full_window[ticker], d)[0] for d in [2,21,63,126]],
                "Trend": [get_perf_and_trend(full_window[ticker], d)[1] for d in [2,21,63,126]]
            })
            t_col.table(p_df)
            ...
            # COLLECT AUDIT FOR THIS SECTION ONLY
            section_audit = []

            for i, cat_name in enumerate(active_cats):
                cat_ticks = [rename_map[t] for t in categories[cat_name].keys()]
                indiv_vals = [corr_matrix.loc[anchor_lbl, lbl] for lbl in cat_ticks]
                avg_val = np.mean(indiv_vals)
                short_name = cat_name.split(': ')[1]
                draw_gauge(f"{category_emojis.get(short_name, '')} {short_name}", avg_val, gauge_cols[i], f"g_{ticker}_{cat_name}_{day_offset}")
                section_audit.append({"Sector": short_name, "Heatmap Indices": ", ".join([f"{v:.2f}" for v in indiv_vals]), "Distilled Avg": f"{avg_val:.2f}"})

            # Gold Singleton
            gold_corr = corr_matrix.loc[anchor_lbl, '🟡 GLD']
            draw_gauge("🟡 Gold", gold_corr, gauge_cols[-1], f"gold_{ticker}_{day_offset}")
            section_audit.append({"Sector": "Gold", "Heatmap Indices": f"{gold_corr:.2f}", "Distilled Avg": f"{gold_corr:.2f}"})

            # INTEGRATED AUDIT TABLE (Per Section)
            with st.expander(f"🧮 {row_title} Math Audit", expanded=False):
                st.dataframe(pd.DataFrame(section_audit), hide_index=True, use_container_width=True)
        ...
        <ul>
            <li><b>The Liquidity Squeeze (TLT vs Tech):</b> High negative correlations indicate a pure valuation/discount-rate squeeze.</li>
            <li><b>The Reflation Trade (Energy vs Banks):</b> Look for high positive synergy; Energy typically drives yields helping Banks.</li>
            <li><b>The Wrecking Ball (DXY):</b> Positive correlation with all assets signals a 'Dollar Milkshake' flight to safety.</li>
            <li><b>The Digital Hedge (VIX vs Crypto):</b> Strong inverse to VIX = 'Digital Gold'. Moving with VIX = High-Beta risk.</li>
        </ul>
        </div>
        ...
        **Example:** Looking at **📉 TLT (Bonds)** vs **💻 Tech**:
        ...
        2. **Right Swing (Red) [+0.5 to +1.0]:** Bonds up, Tech up. Pricing in a Pivot or Growth bid.
        """)
...
with tab_news:
    ...
    for source, url in feed_sources.items():
        ...
        for entry in feed.entries:
            news_items.append({"Source": source, "Date": entry.published if 'published' in entry else "Recent", "Headline": entry.title, "Link": entry.link})
